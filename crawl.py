...
def accept_cookies(page):    # Try on the main page first
    try:
        if find_and_click(page, ACCEPT_WORDS):
            return
    except Exception:
    ...
    for frame in page.frames:
        try:
            if main_frame is not None and frame == main_frame:
                continue
            if find_and_click(frame, ACCEPT_WORDS):
                return
        except Exception:
...
def open_cookie_settings(page):
    try:
        if find_and_click(page, SETTING_WORDS):
            return
    except Exception:
        # If querying the main page fails for some reason, continue to frames
        pass
    ...
    for frame in page.frames:
        try:
            ...
            if find_and_click(frame, SETTING_WORDS):
                return
        except Exception:
            # Accessing cross-origin frame contents or detached frames may raise; skip
            continue
...
def reject_cookies(page):
    ...
    try:
        if find_and_click(page, REJECT_WORDS):
            return
    except Exception:
        # If querying the main page fails for some reason, continue to frames
    ...
    for frame in page.frames:
        try:
            if main_frame is not None and frame == main_frame:
                continue
            # Query buttons and links inside the frame
            if find_and_click(frame, REJECT_WORDS):
                return
        except Exception:
            # Accessing cross-origin frame contents or detached frames may raise; skip
